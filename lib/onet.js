...
const DATA_URL = new URL('../data/onet.json', import.meta.url);
...
/**
 * Occupation code → O*NET's alternate titles for it. These are worded the way
 * postings are ("Market Research Analyst"), not the way SOC taxonomy is
 * ("Market Research Analysts and Marketing Specialists"), which is what makes
 * them the useful side of the data for job matching. Empty for codes our build
 * doesn't carry.
 */
export async function getAltTitles(code) {
  const { byCode } = await load();
  return byCode.get(code)?.alts ?? [];
}
