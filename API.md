...
`title` is required and is still what `matchedOn` echoes back. Board postings carry no SOC codes, so
matching is on titles — but on O*NET's ~60 *alternate* titles for `code` ("Market Research Analyst",
"Social Media Specialist"), which are worded the way postings are, rather than on the canonical
title, which overlaps half the board on words like "Analyst". `code` therefore drives matching too;
for a code our O*NET build doesn't carry, matching falls back to the canonical title alone.
